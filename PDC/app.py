...
def api_co():
    ...
    if certi and Certificate != '' and '.' in Certificate and Certificate.rsplit('.', 1)[1].lower() in ALLOWED_EXTENSIONS:
        ...
        certi.save(os.path.join(app.config['COC'], username + Certificate+extension))
...
def api_extra():
    ...
    if certi and Certificate != '' and '.' in Certificate and Certificate.rsplit('.', 1)[1].lower() in ALLOWED_EXTENSIONS:
        extension = '.' + Certificate.rsplit('.', 1)[1].lower()
        certi.save(os.path.join(app.config['Extra'], username + Certificate+extension))

    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    cursor.execute('INSERT into extracirricular (uid,name,date,type,award,location,certificate) values (%s, %s,%s, %s,%s,%s,%s)', (username,Name_of_activity,Date_of_activity,Type_of_activity,Awards,Location,certi))
    mysql.connection.commit()
    last_id = cursor.lastrowid
    print(last_id)
...
def api_social():
    ...
    if certi and Certificate != '' and '.' in Certificate and Certificate.rsplit('.', 1)[1].lower() in ALLOWED_EXTENSIONS:
        extension = '.' + Certificate.rsplit('.', 1)[1].lower()
        certi.save(os.path.join(app.config['Social'], username + Certificate+extension))

    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    cursor.execute('INSERT into social(uid,name,date,nature,certificate) values (%s, %s,%s, %s,%s)', (username,Name_of_activity,Date_of_activity,Type_of_activity,certi))
    mysql.connection.commit()
    last_id = cursor.lastrowid
    print(last_id)
